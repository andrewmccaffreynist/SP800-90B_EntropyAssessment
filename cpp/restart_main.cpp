...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt(argc, argv, "invoq")) != -1) {
        switch (opt) {
            case 'i':
                iid = true;
                break;
    ...
    if (H_I < 0) {
        ...
        testRun.SetErrorMsg("H_I must be nonnegative.");
        ofstream output;
    ...
    if (H_I > data.word_size) {
        ...
        testRun.SetErrorLevel(-1);
        testRun.SetErrorMsg("H_I must be at most 'bits_per_symbol.");
        ofstream output;
        output.open(outputfilename);
        output << testRun.GetAsJson();
        output.close();
    ...
    if (data.len != MIN_SIZE) {
        ...
        testRun.SetErrorLevel(-1);
        testRun.SetErrorMsg("\n*** Error: data does not contain " + MIN_SIZE + " samples ***\n\n");
        ofstream output;
        output.open(outputfilename);
        output << testRun.GetAsJson();
        output.close();
    ...
    if (cdata == NULL) {
        ...
        testRun.SetErrorLevel(-1);
        testRun.SetErrorMsg("Error: failure to initialize memory for columns");
        ofstream output;
        output.open(outputfilename);
        output << testRun.GetAsJson();
        output.close();
    ...
    if (min(H_r, H_c) < H_I / 2.0 && !quietMode) printf("*** min(H_r, H_c) < H_I/2, Validation Testing Failed ***\n");
    else {
        if (!quietMode) {
